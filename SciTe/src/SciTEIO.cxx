...
#endif
const GUI::gui_char propGlobalFileName[] = GUI_TEXT("全局设置.properties");	//moded
const GUI::gui_char propAbbrevFileName[] = GUI_TEXT("全局缩写.properties");	//moded
...
void SciTEBase::TextRead(FileWorker *pFileWorker) {
	...
	if (iBuffer >= 0) {
		...
		if (pFileLoader->err) {
			GUI::gui_string msg = LocaliseMessage(L"不能打开文件 '^0'.", pFileLoader->path.AsInternal());
			WindowMessageBox(wSciTE, msg);
...
bool SciTEBase::Open(FilePath file, OpenFlags of) {
	...
	if (!absPath.IsUntitled() && absPath.IsDirectory()) {
		GUI::gui_string msg = LocaliseMessage(L"路径 '^0' 是一个目录,因此无法打开.",
			absPath.AsInternal());
		WindowMessageBox(wSciTE, msg);
		return false;
	}
