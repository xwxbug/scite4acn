...
// add start
			...
			strcat(google,findWhat.c_str());
			ShellExecuteA(0,NULL,google ,NULL,NULL,SW_SHOW);
			::EndDialog(hDlg, IDCANCEL);
			wFindReplace.Destroy();
			return FALSE;
		 }
			...
			findWhat = dlg.ItemTextU(IDFINDWHAT);
			CHAR msdn[2048]= "http://social.msdn.microsoft.com/Search/en-US/?Refinement=86&Query=";
			strcat(msdn,findWhat.c_str());
			ShellExecuteA(NULL,NULL, msdn,NULL,NULL,SW_SHOW);
			::EndDialog(hDlg, IDCANCEL);
			wFindReplace.Destroy();
			return FALSE;
		 }
			...
			strcat(iciba,findWhat.c_str());
			::ShellExecuteA(NULL,NULL, iciba,NULL,NULL,SW_SHOW);
			::EndDialog(hDlg, IDCANCEL);
			wFindReplace.Destroy();
			return FALSE;
		 }
...
//add ↓
void SciTEWin::FindMessageBox(const GUI::gui_string &msg, const std::string *findItem) {
	if (findItem == 0) {
		GUI::gui_string msgBuf = LocaliseMessage(msg.c_str());
