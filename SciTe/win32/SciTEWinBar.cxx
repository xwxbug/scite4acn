...
void SciTEWin::Notify(const SCNotification *notification) {
	...
	case NM_RCLICK:
		...
		if (notification->nmhdr.idFrom == IDM_TABWIN) {
			...
			// Pop up menu here:
			popup.CreatePopUp();
			AddToPopUp(L"新建文件", IDM_NEW, true);
			AddToPopUp("");
			...
			AddToPopUp(L"打印文件", IDM_PRINT, true);
...
int substitute(char* s,char chFind, char chReplace) {
	int c = 0;
	char *t = s;
	while (t) {
		t = strchr(t, chFind);
...
void SciTEWin::SetToolBar() {
	...
	std::string sIconlib = props.GetNewExpandString("user.toolbar.iconlib.", fileNameForExtension.c_str());
	HICON hIcon = NULL;
	HICON hIconBig = NULL;
	int iIconsCount = 0;
...
//		HDC hDesktopDC = ::GetDC(NULL);
	...
	char* userToolbarbuff = strdup(userToolbar.c_str());
	size_t userToolbarbufflen = strlen(userToolbarbuff);
	substitute(userToolbarbuff, '|', '\0');
	const char *userContextItem = userToolbarbuff;
	const char *endDefinition = userContextItem + userToolbarbufflen;
		...
		if (userContextItem < endDefinition) {
			if ( tips[0] != '#') {
				...
				if (id > IDM_TOOLS) {
					...
					prefix+= StdStringFromInteger(id - IDM_TOOLS);
					prefix+= ".";
...
//!-start-[user.toolbar]
	...
	if (props.GetInt("toolbar.visible") != 0) {
		...
		for (int i = 0; i < toolbarUsersPressableButtons.GetSize(); i++) {
			std::string prefix = "command.checked.";
			prefix+= StdStringFromInteger(toolbarUsersPressableButtons[i] - IDM_TOOLS);
			prefix+= ".";
			int ischecked = strtoul(props.GetNewExpandString(prefix.c_str(), fileNameForExtension.c_str()).c_str(),0,10);
